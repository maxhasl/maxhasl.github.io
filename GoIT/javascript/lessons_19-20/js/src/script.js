...
$(function() {
  ...
  .jcarouselPagination({
      item: function(page) {
          return '<a class="page" href="#' + page + '"><span class="checked"></span></a>';
      }
  });
...
$('.jcarousel-pagination--2')
  ...
  .jcarouselPagination({
      item: function(page) {
          return '<a class="page--2" href="#' + page + '"><span class="checked--2"></span></a>';
      }
  });
...
var skillsFlattenDeep = _.flattenDeep(skills);
var skillsFlattenUniq = _.uniq(skillsFlattenDeep);
var skillsFlattenUniqSortToLower = _.sortBy(skillsFlattenUniq, _.toLower);
console.log('Skills: ', skillsFlattenUniqSortToLower);
//2
var name = _.map(data, function(e) {
    return { "name": e.name, 
             "friends": e.friends
           }
});
var nameSort = _.map(_.sortBy(name, 'friends.length'), 'name');
console.log('Name by friends: ', nameSort );
//3
var friends = _.map(data, function(e) {
  return {"name": _.map(e.friends, "name")}
});
var friendsMap = _.map(friends, 'name');
var friendsFlattenDeep = _.flattenDeep( friendsMap );
var friendsUniq = _.uniq(friendsFlattenDeep);
var friendsSort = _.sortBy(friendsUniq);
console.log('Friends: ', friendsSort );
});