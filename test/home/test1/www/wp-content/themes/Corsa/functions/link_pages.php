...
function us_wp_link_pages($args = '') {
	...
	$defaults = array(
		...
		'next_or_number' => 'number', 'nextpagelink' => '<i class="fa fa-chevron-right"></i>',
		'previouspagelink' => '<i class="fa fa-chevron-left"></i>', 'pagelink' => '%',
		'echo' => 1
	);
...
if(!function_exists('us_pagination')) {
	function us_pagination($pages = 0, $range = 2)
	{
		...
		if(1 != $pages)
		{
			if($paged > 1) $result .= '<a href="'.get_pagenum_link($paged - 1).'" class="g-pagination-item to_prev"><i class="fa fa-chevron-left"></i></a>';
			...
			if ($paged < $pages) $result .= '<a href="'.get_pagenum_link($paged + 1).'" class="g-pagination-item to_next"><i class="fa fa-chevron-right"></i></a>';
